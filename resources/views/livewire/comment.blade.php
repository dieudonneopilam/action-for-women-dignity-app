    ...
    <div class="card">
        <header class="header-pub">
            <div style="display: flex;">
                <img class="img-profil" src="{{ asset('img/cambg_1.jpg') }}" alt="" srcset="">
                <div>
                    <p class="name-asbl">Action for women dignity</p>
                    <p class="time-pub">{{ $publication->datepub }}</p>
                </div>
            </div>
            @isset(auth()->user()->id)
                @if(auth()->user()->isadmin ==1 || auth()->user()->ismemberadmin==1)
                <p class="optionPub">
                    <svg class="points" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-three-dots-vertical" viewBox="0 0 16 16">
                        <path d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"/>
                    </svg>
                    <svg class="closepub" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
        ...
        @isset(auth()->user()->id)
        <div class="menu-pubOption" style="display: none">
            <ul>
                <li><a href="">view</a></li>
                <li><a href="{{ route('pub.edit', $publication->id) }}">edit</a></li>
                @if(auth()->user()->isadmin == 1)
                <li>
                    <form action="{{ route('pub.destroy',$publication->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button style="background-color: transparent;border: transparent;font-size: 16px">delete</button>
                    </form>
                </li>
                @endif
            </ul>
        ...
        @endisset
        <p class="content-pub">{{ $publication->text }}</p>
        <img class="img-pub" src="{{ Storage::url($publication->file) }}" alt="">
        <footer class="footer-pub">
            @isset(auth()->user()->id)
            <div class="like">
                <?php $vrai = false ?>
                @foreach ($publication->likes as $publike)
                   @if(($publike->user_id == auth()->user()->id) && ($publike->publication_id==$publication->id) )
                   <?php $vrai = true ?>
                   @endif
                @endforeach
                
                @if($vrai)
                <svg color=blue xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16">
                ...
                @else
                <span wire:click = "like({{ $publication->id }} )" >
                ...
                @endif
                <span style="font-size: 14px;margin-left: 10px">{{ $publication->nblike }}</span>
            </div>
